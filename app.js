...
function overallStats(){
  ...
  const keysUnlocked=Math.min(baseKeyCount(),Array.isArray(state.keyring)?state.keyring.length:0);
...
function campaign2Readiness(){
  ...
  if(!g.gates.keys)blockers.push(`${Math.max(0,baseKeyCount()-st.keysUnlocked)} more base Keys to unlock`);
...
function renderEnd(s,before){
  ...
  $("endSub").textContent=`VERB CONTROL ${pct(st.rating)} · ${rb.name} · ${sg.name} · ${Object.keys(state.seen).length.toLocaleString()}/${BANK.length.toLocaleString()} formes vistes · +${s.learningXp||0} XP · COMBO ×${s.bestCombo||0}${s.recovered?` · ${s.recovered} RECOVERED`:""}${s.masteredRewards?` · ${s.masteredRewards} MASTERED`:""}`;
